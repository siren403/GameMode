...
namespace GameMode.Editor.Overlays
{
    ...
    [EditorToolbarElement(id, typeof(SceneView))]
    class GameModesExample : EditorToolbarDropdownToggle, IAccessContainerWindow
    {
        public const string id = "ExampleToolbar/GameModesExample";
        public EditorWindow containerWindow { get; set; }

        private ScriptableGameMode[] _gameModes;
        private int index;

        public GameModesExample()
        {
            text = "Color Bar";
            tooltip =
                "Display a color rectangle in the top left of the Scene view. Toggle on or off, and open the dropdown" +
        ...
        void ShowGameModes()
        {
            _gameModes = AppSettings.Instance.GameModes.ToArray();
            index = Mathf.Clamp(index, 0, _gameModes.Length - 1);

            var menu = new GenericMenu();
            for (int i = 0; i < _gameModes.Length; i++)
            {
                var _i = i;
                var mode = _gameModes[i];
                menu.AddItem(new GUIContent(mode.name), index == _i, () => index = _i);
            }
